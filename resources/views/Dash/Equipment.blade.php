                    ...
                    <tr>
                        ...
                        <td>
                            <div class="description-preview">
                                {{ substr($equipment->description, 0, 100) }} <!-- Display first 50 characters -->
                                @if (strlen($equipment->description) > 100)
                                <span class="show-more" onclick="showFullDescription(this)"><a href="{{ route('equipment.show', $equipment->id) }}" class="show-more">...Show more</a>
                                </span>
                                <span class="full-description">{{ substr($equipment->description, 100) }}</span> <!-- Hidden by default -->
                                @endif
                            </div>
                        </td>
                        ...
                        <td>
                          ...
                          <button class="btn me-2 update-btn"><a href="{{ route('equipment.edit',$equipment->id) }}" class="settings" title="Settings" data-toggle="tooltip" ><i class="fa fa-edit " style="color: rgb(9, 9, 77);  font-size: 18px"></i></a> </button>
                            <form id="delete-form-{{ $equipment->id }}"  method="POST" action="{{ route('equipment.destroy', $equipment->id) }}">
                                @csrf
                                @method('DELETE')
                                <button  type="submit" class="btn btn-danger delete-button" data-delete-id="{{ $equipment->id}}">
